...
display_top_divider(Index, RowSize):-
    put_code(9574), put_code(9552), put_code(9552),put_code(9552),put_code(9552),
    Index2 is Index + 1,
...
display_first_row_cells(Row):-
    length(Row, RowSize),
    RouSizeAux is RowSize + 1,
    display_top_divider(RouSizeAux),
    display_element_row(Row).
...
add_col_char_to_board(GameBoard, NewBoard):-
    ...
    RouSizeAux is RowSize + 1,
    add_col_char_to_board(GameBoard, RouSizeAux, 1, [], NewBoard).
